...
**在本文中你将了解GIL是如何影响python程序的性能和怎样才能减轻这些影响**
...
这个引用计数如果在两个线程同时对它进行增加或减少操作的时候会出现问题，需要保证它不受这种竞争情况的影响。如果不能保证，那么将出现内存泄露甚至是错误地释放一个依然在使用的对象内存的极端情况。这将导致你的python程序崩溃或者是其他莫名其妙的bug。

引用计数可以通过对所有跨线程的数据结构加锁来保证它们不会被改成不一致的情形。

但是，对每个对象或对象组加锁意味着将可能会出现多重锁，而这可能导致另一个问题——死锁（死锁仅会在有超过一个锁的时候发生）。而且不断地获取和释放多个锁也将导致性能降低。

GIL是施加在解释器上的唯一的锁，任何python字节码必须在获取该锁的条件下执行。这样就防止了死锁（因为只有一个锁），而且也不会对性能有太多影响。但是这样就导致所有CPU密集型的python程序变成了单线程。

尽管GIL也被像Ruby等其他语音解释器所使用，但它并不是解决这个问题的唯一办法。有些编程语言使用了诸如垃圾收集而不是引用计数的方式避免依赖GIL来达到线程安全的内存管理。
...
为什么选GIL作为解决方案？
-----
那么，为什么在python中选用这么一个有着明显短板的方案？这是python的开发者的一步臭棋吗？
...
许多python里需要的特性被直接将已有的C库以扩展的形式接入了进来而满足。而这些C扩展依赖GIL提供的线程安全的内存管理，这是为了避免不一致的改动。

GIL易于实现并很轻松地被添加到了python之中。因为只需要管理一个锁，它为了单线程程序带来了性能提升。

非线程安全的C库由此变得更容易整合。这些C扩展也成了python如此轻易地被不同团体采纳的原因之一。

如你所见，GIL对早期的CPython开发者来说是解决这个难题的一个非常务实的办法。
...
当然，也有其他不影响兼容的方案，但是它们其中一些以损失单线程和多线程I/O密集型的性能为代价，另外一些实在是太难以实现。毕竟，你也不会希望你现有的python程序因为新版本地发布而运行地更慢，对吧？
...
------
python3当时是有机会从头开始的，并在那个过程中，打乱了一些已有的C扩展的兼容性，然后为了让它们适应python3增加了一些更新和迁移的工作。这是python3的早期版本社区进展较慢的原因。
...
python GIL经常被当做一个神秘而又高难度的话题。但是请记住作为一个python使用者，你只会在编写C扩展或者是多线程的cpu密集型程序的时候经常被它影响。

总的来说，这篇文章应该已经已经完整地告诉了你什么是GIL和你应该在自己的项目中如何处理它。如果你还想了解GIL的底层工作，我建议你看一下 David Beazley的[《理解python GIL》](https://youtu.be/Obt-vMVdM8s)分享。
